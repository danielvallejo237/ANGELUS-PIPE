...
def run(weights='yolov5s.pt',  # model.pt path(s)
    ...
    # Directories
    save_dir = os.getcwd()  # increment run
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            txt_path = os.path.join(save_dir,p.stem) + ('' if dataset.mode == 'image' else f'_{frame}')  # img.txt
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    if save_txt:  # Write to file
                        ...
                        processed_results.append(ProcessLine(line))
            ...
            if save_txt:
                print('DETECCIÓN DE CLASES DE INTERES')
                for r in processed_results:
                    if len(processed_results)>0:
                        print ("Clase: {}, Coordenadas: {}, Tamaño {}".format(r[0],r[1],r[2]))
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='modelo_final_odet.pt', help='model.pt path(s)')
